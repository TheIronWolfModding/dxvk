...
namespace dxvk {
  ...
  void DxvkContext::clearRenderTarget(
    ...
    if (m_flags.test(DxvkContextFlag::GpRenderPassBound)) {
      ...
      assert(imageView->info().numLayers == 1 ||
             ...
             imageView->info().numLayers == 3); // Multiview.  This could be breaking cube textures.
  ...
  void DxvkContext::generateMipmaps(
    ...
    for (uint32_t i = 0; i < mipGenerator->getPassCount(); i++) {
      ...
      renderingInfo.viewMask = renderingInfo.layerCount == 3 ? 0b111 : renderingInfo.layerCount == 2 ? 0b11 : 0;
